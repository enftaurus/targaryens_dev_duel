...
@router.get("/get_appointments/{student_mail}")
def get_appointments(student_mail: str, request: Request):
    mail=request.cookies.get("user_mail")
    if not mail:
        raise HTTPException(status_code=401, detail="please login to proceed")
    if mail != student_mail:
        raise HTTPException(status_code=403, detail="unauthorized action")
    try:
        response = supabase.table("appointments") \
        .select("*") \
        .eq("student_mail", student_mail) \
        .execute()
        appointments = response.data
    except Exception as e:
        raise HTTPException(status_code=500, detail="failed to fetch appointments")
    return JSONResponse(content={"appointments":appointments}, status_code=200)
@router.post("/")
