...
export default function Appointments() {
  ...
  try {
    const res = await axios.get(
      `http://localhost:8000/cancel_appointment/get_appointments/${user.email}`,
  ...
  const slot = TIME_TO_SLOT[appt.appointment_time];
  ...
  try {
    await axios.post(
      "http://localhost:8000/cancel_appointment/",
      {
        student_mail: appt.student_mail,
  ...
  useEffect(() => {
    const fetchCounsellors = async () => {
      try {
        ...
        const rawList = res.data.counsellors || [];
        // Normalize backend fields: counsellor_image -> img, focus string -> array, creds
        const normalized = rawList.map(c => ({
          ...c,
          img: c.counsellor_image || c.img || "",
          credentials: c.creds || c.credentials || "",
          specialization: c.focus
            ? c.focus.split(",").map(s => s.trim()).filter(Boolean)
            : c.specialization || [],
        }));
  ...
  const onSubmit = async (data) => {
    ...
    const payload = {
      ...
      counsellor_image: selectedCounsellor.img || selectedCounsellor.counsellor_image || "",
      counsellor_creds: selectedCounsellor.creds || selectedCounsellor.credentials || "",
      counsellor_focus: selectedCounsellor.focus || "",
      date: data.date,
      focus_goals: [data.problem],
  ...
  const Step2 = (
    <div className="appt-card">
      ...
      {counsellors.length === 0 ? (
        <div className="alert warn">No counsellors available. Please try again later.</div>
      ) : (
            ...
            return (
              <label key={c.id} className={`c-card ${on ? "on" : ""}`}>
                ...
                <div className="c-meta">
                  ...
                  {c.mail && <div className="text-muted" style={{ fontSize: "0.85rem", marginTop: "2px" }}>{c.mail}</div>}
                  {c.specialization && c.specialization.length > 0 && (
                    <div className="c-tags">
                      {c.specialization.map((t, idx) => (
                        <span className="mini-tag" key={`${c.id}-${idx}`}>
                          {t}
                        </span>
                      ))}
                    </div>
  ...
  <>
      ...
      <div className="container">
        ...
        <div className="stepper">
          {["Details", "Counsellor", "Date", "Time", "Review", "Done"].map((t, i) => (
            <div
              key={t}
              className={`stp ${step === i ? "is-here" : step > i ? "is-done" : ""}`}
            >
    ...
    {/* ===== MY APPOINTMENTS SECTION (NEW – SCROLL DOWN) ===== */}
    <section className="section">
      <div className="container">
        <h3 className="appt-h3">My Appointments</h3>

        {loadingMyAppointments ? (
          <div className="text-center">Loading appointments...</div>
        ) : myAppointments.length === 0 ? (
          <div className="appt-card">
            <p className="text-muted">No active appointments found.</p>
          </div>
        ) : (
            ...
            <div key={appt.id} className="appt-card">
              <div className="card">
                <ul className="kv">
                  <li>
                    <span>Counsellor</span>
                    <strong>{appt.counsellor_name}</strong>
                  </li>
                  <li>
                    <span>Date</span>
